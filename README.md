...
当前附带的 81 条语音在开发时使用清晰、稳定的标准美式英语音色 `en-US-AvaNeural` 和标准普通话音色 `zh-CN-XiaoxiaoNeural` 预生成。每个字母都使用明确的英文名称读法，而不是让语音引擎猜测单个字符；中文以稍慢语速生成，并避免会模糊短音节收尾的二次响度处理。游戏只包含总计约 4.32 MB 的短 MP3 文件，游玩和家长扩词时都不会下载语言包、模型或连接语音服务。第三方说明见 `THIRD_PARTY_NOTICES.md`。
